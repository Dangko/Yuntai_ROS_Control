...
void RxBuffer_Decode(uint8_t* RxBuffer)
{
    for(int i=0;i<20;i++)
    {
        if(RxBuffer[i]==0x66 && RxBuffer[i+1]==0x20)
        {
            int index=i+2;
            angle[0]=((int16_t)(RxBuffer[index]<<8|RxBuffer[index+1]))/100.0;
            angle[1]=((int16_t)(RxBuffer[index+2]<<8|RxBuffer[index+3]))/100.0;
            pose_feedback.orientation.y = angle[1];
            pose_feedback.orientation.z = angle[0];
            flag_decode = true;
        //    ROS_INFO_STREAM("Reading from serial port");
          //  ROS_INFO("pitch:%f",pose_feedback.orientation.y );
            //ROS_INFO("yaw:%f\n",pose_feedback.orientation.z);
            break;
        }
    }
}
